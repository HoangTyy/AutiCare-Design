import React from 'react';

interface FooterProps {
  lang: 'vi' | 'en';
}

const Footer: React.FC<FooterProps> = ({ lang }) => {
  const content = {
    vi: {
      footerDesc: "Hệ sinh thái số hóa can thiệp sớm toàn diện dành cho sự chẩn đoán, sàng lọc và tiến trình hòa nhập bền vững của trẻ em tự kỷ.",
      developersHeader: "Thành viên phát triển",
      mentorHeader: "Giảng viên hướng dẫn",
      linksHeader: "Liên kết nhanh",
      contactHeader: "Thông tin liên hệ",
      home: "Trang chủ",
      category: "Danh mục",
      reviews: "Đánh giá",
      about: "Về chúng tôi",
      contact: "Liên hệ",
      mentorName: "Mentor: Quách Luyl Đa",
      copyright: "Bản quyền © 2026 AutiCare. Bảo lưu mọi quyền."
    },
    en: {
      footerDesc: "Comprehensive early intervention digital ecosystem supporting screening, monitoring, and sustainable social inclusion for children on the autism spectrum.",
      developersHeader: "Development Team",
      mentorHeader: "Project Mentor",
      linksHeader: "Quick Links",
      contactHeader: "Contact Info",
      home: "Home",
      category: "Categories",
      reviews: "Testimonials",
      about: "About us",
      contact: "Contact",
      mentorName: "Mentor: Quach Luyl Da",
      copyright: "Copyright © 2026 AutiCare. All rights reserved."
    }
  };

  const t = lang === 'vi' ? content.vi : content.en;

  const developers = [
    "Lê Nhựt Anh",
    "Huỳnh Hoàng Tỹ",
    "Nguyễn Lê Khắc Vũ",
    "Đặng Công Khanh",
    "Phan việt Phát"
  ];

  return (
    <footer id="footer" className="footer-section">
      <div className="container footer-grid">
        {/* Column 1: Logo & Info */}
        <div className="footer-col footer-info-col">
          <div className="footer-logo neon-text-static">AutiCare</div>
          <p className="footer-desc-text">{t.footerDesc}</p>
          <div className="footer-socials">
            <a href="#" className="social-icon" aria-label="Facebook">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
            </a>
            <a href="#" className="social-icon" aria-label="Twitter">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z"/></svg>
            </a>
            <a href="#" className="social-icon" aria-label="LinkedIn">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>
            </a>
            <a href="#" className="social-icon" aria-label="Github">
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"/></svg>
            </a>
          </div>
        </div>

        {/* Column 2: Developers Team */}
        <div className="footer-col footer-devs-col">
          <h4>{t.developersHeader}</h4>
          <ul className="footer-dev-list">
            {developers.map((dev, idx) => (
              <li key={idx}>
                <span className="dev-dot" />
                <span className="dev-name">{dev}</span>
              </li>
            ))}
          </ul>
        </div>

        {/* Column 3: Mentor & Quick Links */}
        <div className="footer-col footer-links-col">
          <h4>{t.mentorHeader}</h4>
          <div className="mentor-card-footer">
            <span className="mentor-badge-footer">MENTOR</span>
            <div className="mentor-name-text">Quách Luyl Đa</div>
          </div>

          <h4 style={{ marginTop: '2rem' }}>{t.linksHeader}</h4>
          <ul className="footer-link-list">
            <li><a href="#hero">{t.home}</a></li>
            <li><a href="#category">{t.category}</a></li>
            <li><a href="#reviews">{t.reviews}</a></li>
            <li><a href="#about">{t.about}</a></li>
          </ul>
        </div>

        {/* Column 4: Contact info */}
        <div className="footer-col footer-contact-col">
          <h4>{t.contactHeader}</h4>
          <ul className="footer-contact-list">
            <li>
              <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
              <span>contact@auticare.edu.vn</span>
            </li>
            <li>
              <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
              <span>+84 (028) 3930 1212</span>
            </li>
            <li>
              <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
              <span>AutiCare Central, Q3, TP.HCM</span>
            </li>
          </ul>
        </div>
      </div>

      <div className="footer-bottom container">
        <p className="copyright-text">{t.copyright}</p>
      </div>
    </footer>
  );
};

export default Footer;
