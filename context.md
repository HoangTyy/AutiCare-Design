...
- **Interactive Objectives Table (Funtopia 3D Floating Rows)**: Thiết kế lại toàn bộ cụm tiêu đề mục tiêu con thành một **Header Card Memphis thu nhỏ** viền Slate đen dày `3px`, nền trắng, bóng đổ 3D `4px 4px 0px #1E293B` cực kỳ vững chãi. Bảng Objectives (`.objectives-table`) được cấu hình `border-collapse: separate` với khoảng giãn cách dòng nổi `border-spacing: 0 12px` bồng bềnh. Mỗi dòng chính `.obj-main-row` có viền Slate dày `2.5px solid #1E293B`, nền trắng, bóng đổ 3D offset `4px`, tự động nẩy nổi tịnh tiến `translate(-3px, -3px)` và giãn bóng đổ lên `7px` kèm lề trái Violet đậm đà khi hover. Nút đóng/mở dòng được thiết kế dạng **Sticker Circle Button Memphis mini** hình tròn tự chuyển màu tím Violet và nẩy nhẹ khi hoạt động. Hàng phụ mở rộng được dọn sạch viền Slate và bóng đổ để lộ trọn vẹn card con `.activity-section-wrapper` màu vàng kem thô ráp Memphis vô cùng nghệ thuật bên dưới.
