...
## Design Philosophy
- **Two-way HTML5 Hash Routing**: Toàn bộ hệ thống SPA sử dụng giải pháp Hash Router (`#/`, `#/dashboard/admin`, `#/profile`, `#/staff-profile`, `#/dashboard/staff`, `#/centers`) đồng bộ hai chiều với React `view` state. Các nút chuyển đổi vai trò và điều hướng đều dùng thẻ `<a>` thật để trình duyệt hiển thị liên kết trực quan dưới góc màn hình, tăng tính chuyên nghiệp, hỗ trợ mở tab mới và khả năng Back/Forward của trình duyệt mà không reload.
...
- **Interactive Activity Detail Modal**: Thiết kế Modal Chi tiết Hoạt động ở chế độ xem (`actModalMode === 'view'`) thành một trung tâm tương tác 2 cột Funtopia/Memphis rộng lớn **920px** (`width: actModalMode === 'view' ? '920px' : '600px'`) có bóng đổ cứng Slate đen 3D lộng lẫy. Cột bên trái hiển thị các thông tin tĩnh của bài tập (tần suất, người thực hiện, phương pháp giảng dạy, tiêu chí đạt). Cột bên phải phản ứng động theo vai trò giả lập: Phụ huynh có thể nộp Check-in bài tập thực hành (uploader Base64, ghi chú và nút điền mẫu ma thuật `🪄`), Chuyên gia (giáo viên/bác sĩ) có thể xem bài nộp mới nhất và chấm nhận xét chuyên môn `expert_feedback`. Phía dưới cùng là một **Timeline Lịch sử thực hành & Đánh giá kẹp đôi** đứt nét Memphis kết nối trực quan toàn bộ các lần nộp bài của con và nhận xét chuyên môn của giáo viên, giúp phụ huynh và giáo viên luôn nắm bắt được quá trình phát triển của con một cách sinh động nhất.
- **Interactive Objectives Table (Funtopia 3D Floating Rows & Directional Shadows)**: Thiết kế lại toàn bộ cụm tiêu đề mục tiêu con thành một **Header Card Memphis thu nhỏ** viền Slate đen dày `3px`, nền trắng, bóng đổ 3D `4px 4px 0px #1E293B` cực kỳ vững chãi. Bảng Objectives (`.objectives-table`) và bảng Hoạt động con (`.activity-sub-table`) được cấu hình `border-collapse: separate` với khoảng giãn cách dòng nổi bồng bềnh. Để đảm bảo dải bóng đổ (shadow) dưới đáy các dòng hiển thị liên tục, mịn màng và không bị sọc dọc đen thô kệch chen giữa các ô kề nhau, hệ thống sử dụng **Shadow Định hướng**: Các ô `td` ở giữa chỉ đổ bóng thẳng xuống dưới (`box-shadow: 0px 4px 0px #1E293B` cho bảng lớn và `0px 3px 0px #1E293B` cho bảng con), và duy nhất ô ngoài cùng bên phải `td:last-child` mới đổ bóng chéo sang phải và đáy. Mỗi dòng chính `.obj-main-row` có viền Slate dày `2.5px solid #1E293B`, nền trắng, tự động nẩy nổi tịnh tiến `translate(-3px, -3px)` và giãn bóng đổ đáy lên `7px` (ô cuối cùng là `7px 7px`) kèm lề trái Violet đậm đà khi hover. Để bảo vệ các bóng đổ tuyệt đẹp này khỏi bị cắt cụt bởi container cuộn ngang (`overflow-x: auto`) khi responsive di động, container bọc ngoài `.objectives-table-wrapper` được thiết lập một **Khoảng đệm (Shadow Buffer)** bằng padding tinh tế `padding: 8px 14px 16px 8px !important;`. Khung thead của cả hai bảng cũng được bọc viền Slate đen dày dặn bao quanh toàn bộ (`border-top/bottom: 3px solid #1E293B`, cột đầu/cuối có viền trái/phải và bo góc tròn) và phẳng sạch sẽ 100% không sọc dọc ở giữa. Nút đóng/mở dòng được thiết kế dạng **Sticker Circle Button Memphis mini** hình tròn tự chuyển màu tím Violet và nẩy nhẹ khi hoạt động. Hàng phụ mở rộng được dọn sạch viền Slate và bóng đổ để lộ trọn vẹn card con `.activity-section-wrapper` màu vàng kem thô ráp Memphis vô cùng nghệ thuật bên dưới, có padding đáy `24px` để ôm trọn bóng đổ của dòng hoạt động con cuối cùng.
